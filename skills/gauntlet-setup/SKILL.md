...
git status --short .claude/skills/gauntlet-setup/
```

**`git status` が空なら手元の skill が最新なので、そのまま下へ。行が出たら、書き換わった
`SKILL.md` を読み直してから下へ** — いま読んでいるのは上書きされる前の本文で、新しい版が
要求する後始末はそこに書かれていない。記録の形式が変わったことも配線が変わったことも
知らないまま緑に見える、というのがこれを飛ばしたときの失敗の形。

skill に版番号は無い（lock が持つのは内容ハッシュだけ）ので、比較するのは版ではなく
**入れ直して差分が出たかどうか**。`add` は冪等なので、調べてから打つより先に打つ方が短い。
