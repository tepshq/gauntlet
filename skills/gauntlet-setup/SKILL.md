...
>
> 入れるものは devDependencies が 1 つと、設定ファイルが 3 つ（`gauntlet.config.json` /
...
**間接依存が同じ理由で弾かれることがある**（h3 で実例あり）。
自動では除外されないので、名指しされたものを
...
`tests.projects` に**宣言された project だけ**を走らせる（実行も coverage も。
...
- **`timeout-minutes` は既存のテスト job と同じ桁で足りる。** `full` は
  全テスト + coverage + 型チェック + 重複（jscpd）で、支配項は全テストの実行時間
...
CRAP 違反や重複を潰したあと、**`full` を 2 回回さないこと。** 記録が締まるのは
clean なツリーの実測だけなので（作業途中の値を基準にしないため）、コミット前に測ると
その計測は捨てられ、コミット後にもう一度測ることになる。

```
潰す → git commit → npx gauntlet full   # ここで測って、そのまま記録される
```

得られる情報は同じで、計測は 1 回。落ちたら直してまたコミットする。
...
記録（`gauntlet.baseline.json`）の編集が要る項目では、**エージェントは編集できない**
（guard が書き換えを止める。緩める経路を塞ぐ仕組みに、更新のための例外を開けない）。
**該当したらユーザーに編集を頼み、そのあと `full` を回す。**

- **0.27.0 で mutation ゲートを外した**（実行時間が理由。DESIGN §2）。0.26 以前から
  上げたリポジトリでは:
  - `@stryker-mutator/core` / `@stryker-mutator/vitest-runner` は不要になったので外してよい
  - CI や手順書に `gauntlet doctor` / `gauntlet seed` の呼び出しが残っていれば消す
    （呼ぶと「0.27.0 で外れました」で exit 2 になる）
  - 記録の `mutation` の欄は**読まれなくなった**。次に記録が動いた回に自動でファイルから
    消える。残っていても害は無いので、手で消す必要も無い
...
- **`full` が「リポジトリ全体の違反が 0 → N に増えました」で恒久的に落ちる場合**、
  ...
  種置きに戻る）。0.24.1 以降は、計測を完了しなかったゲートの欄は書かれない
