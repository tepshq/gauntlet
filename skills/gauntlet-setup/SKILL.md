...
- **`timeout-minutes` が足りるか。** `full` は全テスト + mutation なので、既存の job の
  timeout では足りないことがある。**実測（duct）: ソース 2 + テスト 2 ファイルの差分が
  変異対象 23 ファイル・4897 変異になり、手元 11 分 20 秒・CI（4 core）は 58 分で
  終わらなかった**（60 → 150 分に引き上げて対処）。結合寄りのテスト（モックを使わない）を
  1 つ触ると、そのテストが覆うソースが全部対象に入るのが理由で、**差分の大きさと
  mutation の費用は結びつかない**。CI のテストが 10 分なら mutation 込みで 30 分では
  足りないことがある、と見て決める。**打ち切られても手掛かりは残る** — mutation 中は Stryker の進行
  （`Mutation testing 41% … 643/1835 tested`）がログに流れるので、次に何分にすべきかは
  そこから決められる
...
重複はファイルの対とトークン数で出るので、そのまま開いて読める。
ここで未参照コードや網羅率 0 の公開 API が見つかることがある（h3 では 3 件）。
直すかどうかは導入とは別の判断なので、一覧を見せて終わりでよい。

### mutation の記録を置く

**`full` を回しても `mutation` は `{}` のまま。** 変異対象は差分から決まるので、clean な
既定ブランチでは必ず 0 件になる。このまま進めると、**最初にそのファイルに触れた PR が
自分の新コードの生き残りを許容値にする**（報告例では 1 ファイル 513 件）。main の状態を
先に記録しておく:

```
npx gauntlet seed --mutation='<測る範囲の glob>'
```

- **clean なツリーで、既定ブランチで回す**（記録するのはコミット済みの実測だけ）
- **記録の無いファイルだけ測る**ので、範囲を変えて何回かに分けて回せる。大きいリポジトリは
  `lib/**` → `app/**` のように分ける。同じコマンドを繰り返すと残りだけが進む
- 記録が置かれたらコミットする（`git add -A` でよい）
- 時間は `full` の mutation と同じ桁。**変異対象 23 ファイルで手元 11 分・CI 58 分**という
  実測があるので、範囲は小さく切って始める

**完了条件** — `gauntlet.baseline.json` の `mutation` が空でないこと。空のまま進めるなら、
「そのリポジトリでは mutation ゲートは当面噛まない」とユーザーに伝えて合意を取る。
...
`npx gauntlet doctor` が通ること。**`mutation` が空でないこと**（上の `seed`）。
...
- **`full` が「リポジトリ全体の違反が 0 → N に増えました」で恒久的に落ちる場合**、
  ...
  計測できません」）。**触っていないのに大量に増えたと言われるのが目印。**
  消してもらうのは **`"crap"` の 1 行だけ**。clean なツリーで `full` を回すと
  「許容値の記録を作りました」で置き直される（欄が無い = まだ測っていない、なので
  種置きに戻る）。記録を丸ごと消すと mutation の種まで巻き添えになり、下の `seed` を
  回し直すことになる。0.24.1 以降は、計測を完了しなかったゲートの欄は書かれない
- **0.24.5 より前に導入したリポジトリは、mutation の種を置く手段が無かった**
  （`seed` は 0.24.5 で入った）。記録にあるのは「最初にそのファイルに触れた PR」が
  作った分だけで、残りは**次に触った PR が自分の新コードの生き残りを許容値にする**。
  上げたあと、clean な既定ブランチで一度回す:

  ```
  npx gauntlet seed --mutation='<測る範囲の glob>'
  ```

  記録の無いファイルだけを測るので、既にある記録は動かない。置くものが無ければ
  「記録を置く対象がありませんでした」で終わるので、当たっているかを先に調べなくてよい。
  時間と分割の目安は「mutation の記録を置く」（導入の 6）と同じ
