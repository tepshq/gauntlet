...
**main 直コミット運用では mutation の記録が締まらない（2026-08-12 に実測）。**
...
PR 運用なら窓があるので噛み合う。gauntlet 自身が直コミットなので自分だけが踏んでいる、
という可能性が高い — **チームの repo（PR 運用）で本当に噛み合うかを確かめてから**、
機構を足すかどうかを決める。候補: `full --record` のような明示の記録回、
または差分が空のとき記録済みファイルを対象にする。
