...
**この回で作業ツリーの事故も踏んだ。** 別のセッションが同じリポジトリを触っていて、
Stryker の `--inPlace`（実行前に全体を退避し、終了時に書き戻す）が衝突し、
こちらの未コミットの編集が消えた。DESIGN の「mutation は CI でしか走らせないので
作業ツリーは使い捨て」という前提が、`doctor` と手元 `full` を足したことで崩れている。
→「一巡したら決めたいこと」に追加。
