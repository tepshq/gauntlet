...
**`quick` の起動点はリポジトリが選ぶ。** exit 2 は Stop フック（停止を阻止）とも
git の pre-commit（コミットを阻止）とも両立する。

- **速いリポジトリ（数秒）**: `Stop` フック。毎ターン検査、赤ならエージェントが終了できない。`init` の既定
- **遅いリポジトリ（数十秒。duct が実例 — 実差分 25〜30 秒で毎ターンは成立しない）**:
  **pre-commit**。コミットが検問所になり、「履歴に入った状態はすべて検査済み」という
  不変条件になる。頻度は作業の節目に一致し、赤ならコミットできないので強制は残る。
  既知の穴は `git commit --no-verify`（guard で塞ぐ判断は導入時に行う — pre-commit に
  gauntlet 以外のフックがあるリポジトリでは一律に塞げない）
