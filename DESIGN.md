...
**`quick` の起動点は pre-commit（全リポジトリ共通の基本）。** `init` が
`.githooks/pre-commit` を書き、配線は clone ごとに一度 `git config core.hooksPath .githooks`。
コミットが検問所になり、「**履歴に入った状態はすべて検査済み**」が不変条件になる。
頻度は作業の節目に一致し、赤ならコミットできないので強制が残る。
duct 本導入（#567）で実証してから、0.13 で既定に昇格した。

当初は「速いリポジトリは `Stop` フック（毎ターン）、遅いリポジトリは pre-commit」と
分けていたが、**一本化した** — 起動点がリポジトリごとに違うと緑の意味の説明が 2 種類になり、
duct の実測（実差分 25〜30 秒）で Stop が成立しないリポジトリが現実に存在する以上、
全リポジトリで成立する方に揃える。速いリポジトリが毎ターン検査を失う代償は、
コミット粒度が細かければ実質的に小さい。exit 2 は Stop フックとも両立するので、
毎ターン検査に戻したいリポジトリは自分で `.claude/settings.json` に足せる（gauntlet は配らない）。

既知の穴は `git commit --no-verify`。guard で塞ぐかは**破られた証拠が出てから**決める
（pre-commit に gauntlet 以外のフックがあるリポジトリでは一律に塞げない。
「2 つ目の仕組みは 1 つ目が破れた証拠を出してから」の原則どおり）。
