...
| `full` | CI（PR をブロック） | 全テスト + 全体 coverage + CRAP + lint + 重複 + mutation | 分単位可 |
...
eslint や Stryker と違い対象側の何にも合わせる必要が無いので、同梱できる。
...
**coverage・CRAP・mutation・重複の 4 つ**（+ 対象リポジトリの lint と型チェック）。対象は:
