...
| 重複（duplication）ゲート | 信号の実在は jscpd で実測済み（duct: 377 クローン・重複行 3.17%、route の約 100 行丸写し等）だが、今は持たないと決めた。エージェントのコピペはどのゲートも見ていない（§7-8）。再検討するなら「`pr` のみ・リポジトリ全体の重複トークン数 1 つを baseline ratchet・jscpd は gauntlet が同梱してバージョン固定」の形 |
...
8. **コピペはどのゲートも見ていない。** テストごと複製すれば CRAP も mutation も通る。
   重複ゲートは検討の上で外した（§6。duct の実測: 重複行 3.17%）。この次元の劣化は
   黙って進む — 実害が繰り返し出たら §6 の再検討の形で受ける。
